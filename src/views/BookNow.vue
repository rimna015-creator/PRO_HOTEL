<template>
  <div class="min-h-screen bg-blue-50 px-6 pb-16 pt-24">
    ...
    <div v-if="!hotelDetail" class="flex flex-col items-center justify-center py-20 text-center">
      <i class="bi bi-exclamation-triangle text-5xl text-blue-400"></i>
      <h2 class="mt-4 text-xl font-semibold text-blue-900">{{ t("Hotel Not Found") }}</h2>
      <p class="mt-2 text-sm text-blue-500">{{ t("The hotel you are trying to book does not exist.") }}</p>
    ...
    <div v-else-if="bookingConfirmed && lastBooking" class="mx-auto max-w-2xl">
      <div class="rounded-2xl bg-white p-8 text-center shadow-md">
        ...
        <h1 class="mt-5 text-3xl font-bold text-blue-900">{{ t("Thank You!") }}</h1>
        <p class="mt-3 text-lg text-blue-700">
          {{ t("Your booking has been confirmed successfully.") }}
        </p>

        <div class="mx-auto mt-6 max-w-md rounded-xl bg-emerald-50 p-5 text-left">
          <div class="flex items-center justify-between border-b border-emerald-100 pb-3">
            <span class="text-sm text-blue-500">{{ t("Booking ID") }}</span>
            <span class="font-mono text-sm font-bold text-emerald-700">{{ lastBooking.id }}</span>
          </div>
          <div class="flex items-center justify-between border-b border-emerald-100 py-3">
            <span class="text-sm text-blue-500">{{ t("Hotel") }}</span>
            <span class="text-sm font-semibold text-blue-900">{{ lastBooking.hotelName }}</span>
          </div>
          <div class="flex items-center justify-between border-b border-emerald-100 py-3">
            <span class="text-sm text-blue-500">{{ t("Room") }}</span>
            <span class="text-sm font-semibold text-blue-900">{{ lastBooking.roomType }}</span>
          </div>
          <div class="flex items-center justify-between border-b border-emerald-100 py-3">
            <span class="text-sm text-blue-500">{{ t("Dates") }}</span>
            <span class="text-sm font-semibold text-blue-900">
              {{ formatDate(lastBooking.checkIn) }} - {{ formatDate(lastBooking.checkOut) }}
            </span>
          </div>
          <div class="flex items-center justify-between pt-3">
            <span class="text-sm text-blue-500">{{ t("Total") }}</span>
            <span class="text-lg font-bold text-blue-900">${{ lastBooking.price }}</span>
          </div>
          ...
          <div class="mt-2 flex items-center justify-between">
            <span class="text-sm text-blue-500">{{ t("Payment Method") }}</span>
            <span class="text-sm font-semibold text-blue-900">{{ lastBooking.paymentMethod }}</span>
          </div>
          <div v-if="(lastBooking.balanceDue ?? 0) > 0" class="flex items-center justify-between">
            <span class="text-sm text-blue-500">{{ t("Balance at Check-in") }}</span>
            <span class="text-sm font-semibold text-emerald-600">${{ lastBooking.balanceDue }}</span>
          </div>
        </div>
        ...
        <div class="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
          <RouterLink
            to="/my-booking"
            class="rounded-lg bg-emerald-600 px-6 py-2.5 font-medium text-white transition hover:bg-emerald-700"
          >
            {{ t("View My Bookings") }}
          </RouterLink>
          <RouterLink
            :to="'/hotel/' + lastBooking.hotelId"
            class="rounded-lg bg-white px-6 py-2.5 font-medium text-blue-700 transition hover:bg-blue-200"
          >
            {{ t("Back to Hotel") }}
          </RouterLink>
        </div>
    ...
    <div v-else class="mx-auto max-w-3xl">
      ...
      <div class="overflow-hidden rounded-2xl bg-white shadow-md">
        <div class="flex flex-col items-center gap-5 p-6 sm:flex-row">
          ...
          />
          <div>
            <h1 class="text-2xl font-bold text-blue-900">{{ hotelDetail.name }}</h1>
            <p class="flex items-center gap-1 text-sm text-blue-500">
            ...
            <p class="mt-1 flex items-center gap-1 text-sm text-amber-500">
              ...
              <span class="text-blue-500">({{ hotelDetail.reviewer }} {{ t("reviews") }})</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="mt-6 rounded-2xl bg-white p-6 shadow-md sm:p-8">
        <h2 class="mb-5 text-xl font-semibold text-blue-900">{{ t("Guest Information") }}</h2>

        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">

          <!-- Full Name -->
          <div class="sm:col-span-2">
            <label for="fullName" class="block text-sm font-semibold text-blue-700">{{ t("Full Name") }}</label>
            <input
              id="fullName"
              v-model="fullName"
              type="text"
          ...
          <!-- Email -->
          <div>
            <label for="email" class="block text-sm font-semibold text-blue-700">{{ t("Email Address") }}</label>
            <input
              id="email"
              v-model="email"
              type="email"
          ...
          <!-- Phone -->
          <div>
            <label for="phone" class="block text-sm font-semibold text-blue-700">{{ t("Phone Number") }}</label>
            <input
              id="phone"
              v-model="phone"
              type="tel"
          ...
          <!-- Check-in -->
          <div>
            <label for="checkIn" class="block text-sm font-semibold text-blue-700">{{ t("Check-in Date") }}</label>
            <input
              id="checkIn"
              v-model="checkIn"
              :min="today"
              type="date"
              class="mt-2 block w-full rounded-xl border border-blue-200 bg-white px-4 py-3 text-blue-900 focus:border-blue-400 focus:bg-white focus:ring-2 focus:ring-blue-200"
            />
          </div>

          <!-- Check-out -->
          <div>
            <label for="checkOut" class="block text-sm font-semibold text-blue-700">{{ t("Check-out Date") }}</label>
            <input
              id="checkOut"
              v-model="checkOut"
              :min="minCheckOut"
          ...
          <!-- Room Type -->
          <div>
            <label for="roomType" class="block text-sm font-semibold text-blue-700">{{ t("Room Type") }}</label>
            <select
              id="roomType"
              v-model="roomType"
              class="mt-2 block w-full rounded-xl border border-blue-200 bg-white px-4 py-3 text-blue-900 focus:border-blue-400 focus:bg-white focus:ring-2 focus:ring-blue-200"
          ...
          <!-- Guests -->
          <div>
            <label for="guests" class="block text-sm font-semibold text-blue-700">{{ t("Guests") }}</label>
            <input
              id="guests"
              ...
              class="mt-2 block w-full rounded-xl border border-blue-200 bg-white px-4 py-3 text-blue-900 focus:border-blue-400 focus:bg-white focus:ring-2 focus:ring-blue-200"
            />
            <p v-if="selectedRoom" class="mt-1 text-xs text-blue-500">
        ...
        <div class="mt-6 rounded-xl bg-emerald-50 p-5">
          ...
          <div class="flex items-center justify-between">
            <span class="font-semibold text-blue-900">{{ t("Total Price") }}</span>
            <span class="text-2xl font-bold text-blue-900">${{ totalPrice }}</span>
          </div>
        </div>
